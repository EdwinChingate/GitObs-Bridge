...
`ExtractCodeblock_GitBridge` orchestrates the logic described in this module. It calls `Path`, `extract_code_block` as part of its workflow.
...
- `FileLoc`: used in expressions such as `extract_code_block(Path(FileLoc), section_header=section_header)` to derive runtime paths.
- `section_header`: used in expressions such as `extract_code_block(Path(FileLoc), section_header=section_header)` to derive runtime paths.
...
- `extract_code_block`: helper function invoked inside the workflow.
