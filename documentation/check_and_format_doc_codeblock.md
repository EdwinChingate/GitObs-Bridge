...
`check_and_format_doc_codeblock` formats the fenced code block inside a documentation file, optionally rewrites the block, and reports syntax status. It prepares helper paths such as `path`. It calls `Path`, `autopep8`, `extract_code_block`, `replace_code_block`, `shutil`, `subprocess` as part of its workflow.
...
- Calls `autopep8` to delegate work.
- Calls `shutil` to delegate work.
- Calls `subprocess` to delegate work.
- Calls `compile` to delegate work.

---
## Parameters

- `doc_path`: user supplied argument consumed directly by `check_and_format_doc_codeblock`.
- `apply_changes`: user supplied argument consumed directly by `check_and_format_doc_codeblock`.
...
- `{'language': reported_language, 'formatted_code': formatted, 'syntax_ok': syntax_ok, 'syntax_error': syntax_error}`
...
- `autopep8`: helper function invoked inside the workflow.
- `shutil`: helper function invoked inside the workflow.
- `subprocess`: helper function invoked inside the workflow.
- `compile`: helper function invoked inside the workflow.
