...
`update_documentation_file` parses a documentation file and fills its template sections based on the referenced function implementation. It prepares helper paths such as `path`, `repo_path`, `code_path`. It calls `Path`, `CodeBlock`, `extract_code_block`, `ast`, `re` as part of its workflow. The function iterates through runtime collections via `for node in tree.body` and `for py_file in repo_root_path.rglob('*.py')`.
...
- Constructs `path` from `Path(doc_path)`.
- Constructs `repo_path` from `Path(repo_root)`.
- Constructs `code_path` from `repo_path / functions_subfolder / f"{path.stem}.py"`.
- Loops via `for node in tree.body` to process runtime collections.
- Loops via `for py_file in repo_root_path.rglob('*.py')` to process runtime collections.
- Calls `extract_code_block` to delegate work.
- Calls `ast` to delegate work.
- Calls `re` to delegate work.

---
## Parameters

- `doc_path`: user supplied argument consumed directly by `update_documentation_file`.
- `repo_root`: user supplied argument consumed directly by `update_documentation_file`.
- `functions_subfolder`: user supplied argument consumed directly by `update_documentation_file`.

---
## Input

- `path`: derived from `Path(doc_path)` to keep track of resources.
- `repo_path`: derived from `Path(repo_root)` to keep track of resources.
- `code_path`: derived from `repo_path / functions_subfolder / f"{path.stem}.py"` to keep track of resources.
...
- [`extract_code_block`](extract_code_block.md): extracts fenced code blocks and metadata from markdown documentation.
- `ast`: helper function invoked inside the workflow.
- `re`: helper function invoked inside the workflow.

---
## Called by

- [`update_all_documentation`](update_all_documentation.md) uses this helper.
