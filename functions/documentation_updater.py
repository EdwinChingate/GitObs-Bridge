"""Generate and fill documentation sections based on the DocumentationTemplate."""
...
from .ExtractCodeblock_GitBridge import CodeBlock, extract_code_block
...
def _parse_first_function(code: str, preferred_name: Optional[str] = None) -> Optional[ast.FunctionDef]:
    try:
        tree = ast.parse(code)
    except SyntaxError:
        return None
...
def _find_callers(function_name: str, repo_root: Path, current_file: Path) -> List[str]:
...
def _build_sections(info: FunctionInfo, repo_root: Path, code_path: Path) -> Dict[str, str]:
    callers = _find_callers(info.name, repo_root, code_path)
    sections = {
